    ...
    for event in pygame.event.get():
      if event.type==pygame.QUIT:
        ...
        if event.key==pygame.K_UP:
          ...
          """
          행 단위로 실행
          각 행의 숫자가 부여된(0이 아닌) 블럭들 중, 맨 오른쪽 블럭의 숫자와 그 블럭의 왼쪽에 있는 블럭의 숫자가 일치하면
          오른쪽 블럭의 값을 2배 시켜준 후 왼쪽 블럭의 값을 0으로 만들고, 
          ...
          while (mov가 k보다 작은 경우)
            mov번째 블럭을 오른쪽에서 mov번째 위치로 옮김 // mov가 1인 블럭은 맨 오른쪽 블럭이고, 오른쪽에서 1번째 위치, 즉 맨 오른쪽으로 옮긴다        
          """
